...
The publish workflow will automatically push preview packages to **GitHub Packages** on every push to `main`. No secrets are required for this to work, but the repository must have **GitHub Packages** enabled (it is by default).
...
3. Set the **tag** to the exact version string (e.g. `1.5.0`). Create the tag on `main` (or the appropriate commit).
